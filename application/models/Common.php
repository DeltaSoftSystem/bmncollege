...
class Common extends CI_Model
{
        ...
        if ($filename != '') {
            ...
            if (!in_array($extension, $file_allowed)) {
                ...
                if (!is_uploaded_file($_FILES[$fileField]['tmp_name'])) {
                    $returnArray = array("uploaded" => "false", "uploadMsg" => "Temporary file missing or invalid", "imageFile" => "");
                    return $returnArray;
                }

                if (!is_writable($path)) {
                    $returnArray = array("uploaded" => "false", "uploadMsg" => "Upload directory is not writable", "imageFile" => "");
                    return $returnArray;
                }

                $copied = copy($_FILES[$fileField]['tmp_name'], $newname);
                if (!$copied) {
                    $returnArray = array("uploaded" => "false", "uploadMsg" => "Failed to copy file to: $newname", "imageFile" => "");
                    return $returnArray;
                }
        ...
        if ($tablename != "") {
            ...
            if ($query->num_rows() > 0) {
                ...
                if ($rowsdata == "multiple") {
                    ...
                    if (isset($resultdata['value']) && $resultdata['value'] != "") {
                        if (file_exists("/uploads/" . $folder . "/" . $resultdata['value'])) {
                            $resultdata['value'] =  "/uploads/" . $folder . "/" . $resultdata['value'];
                        } else {
    ...
    public function load_lang($siteLang = 'english')
    {
    ...
    public function getLanguageExtOther($LANGCODE = "EN")
    {
        if (strtolower($LANGCODE) == "en" || strtolower($LANGCODE) == "english") {
            return "english";
        } else {
            return "marathi";
        }
