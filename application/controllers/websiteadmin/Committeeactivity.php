...
class Committeeactivity extends CI_Controller
{
    //   public $db;
    public $title = "Committee Activity";
    public $controller = "websiteadmin/committeeactivity";
    ...
    public function listshow($start = 0, $otherparam = "")
    {
        ...
        $data['sub_activaation_id'] = '1022_2';
        $data['title'] = 'Committee-activity';
        $data['start'] = $start;
        $data['maxm'] = $maxm = 50;
        $data['sub_heading'] = 'List';
        $fun_name = $this->controller . '/listshow';
        $data['fun_name'] = $fun_name;
        $data['add_link'] =  $this->controller . '/committeeactivity_action';
        $data['edit_link'] =  $this->controller . '/committeeactivity_action';
        ...
        $this->load->view('websiteadmin/committeeactivity_listall_view', $data);
    ...
    public function committeeactivity_action($id = "")
    {
        ...
        $data['sub_heading'] = (isset($id) && $id != "") ? " Edit Committee-activity" : 'Add Committee-activity';
        $data['back_link'] =  $this->controller . '/listshow';
        $data['fun_name'] = 'committeeactivity_action/' . $id;
        $data['controller'] = $this->controller;
        ...
        if (isset($_POST['mode']) && $_POST['mode'] == "edit_content") {
            ...
            $event_time = (isset($_POST['event_time'])) ? $this->common->mysql_safe_string($_POST['event_time']) : '';
            ...
            $add_in_m['event_time'] = $event_time;
            ...
            if (isset($_FILES['main_image']['name']) && $_FILES['main_image']['name'] != '') {
                ...
                $filename = "committee-activity-".time() . $this->common->tep_short_name_list($_FILES['main_image']['name']);

                $upload = $this->common->UploadImage('main_image', 'uploads/events/', 0, $height_thumb = '', $width_thumb = '', $filename);
            ...
            if ($error == '') {
                ...
                if (!empty($event_dates)) {
                    ...
                    //  $add_in_m['event_time'] = $event_dates;
                }

              if ($heading_old != $heading) {
                  $add_in['slug_name'] = $this->common->getUniqueSlug('wti_m_committee_activity', 'slug_name', $this->common->tep_short_name_list($heading), 'slug_name');
              }

               
                if ($id != "") {
                  $where = "uuid = '" . $id . "'";
                  $update_status = $this->common->updateRecord('wti_m_committee_activity', $add_in_m, $where);
                ...
                if($add_in_m['status_flag']==1){
                    ...
                    $add_in_search['section_name'] = "committee-activity";
                    $add_in_search['section_auto_id'] = $id;
                    $add_in_search['table_name'] = "wti_m_committee_activity";
                    $add_in_search['contents'] = $add_in_m['heading']."<br>".$add_in_m['descriptions'];
                    $add_in_search['urls'] = site_url('committee-activity');
                    $add_in_search['main_url'] =  site_url('committee-activity');
                ...
                $blogs_id = $this->common->insertRecord('wti_m_committee_activity', $add_in_m);
                ...
                //$this->common->createjson('wti_m_committee_activity', 'news');
        ...
        $data_info = array();
        if ($id != "") {
            $sql = "select * from  wti_m_committee_activity b where   b.delete_status=0 and b.uuid='" . $id . "'   order by  b.newsblogs_id desc";
            $query = $this->db->query($sql);
            if ($query->num_rows() > 0) {
            ...
            if (isset($add_in_m) && sizeof($add_in_m) > 0) {
                ...
                $sql = "select count('')  as numrows  from wti_m_committee_activity        where     (delete_status=0 or delete_status IS NULL)   ";
                $query = $this->db->query($sql);
                $resultdata = $query->row_array();
        ...
        $this->load->view('websiteadmin/committeeactivity_action', $data);
        $this->session->unset_userdata('success');
        $this->session->unset_userdata('warning');
        $this->session->unset_userdata('error');
    ...
    public function delete_data($id = 0)
    {
        ...
        $sql = "select  uuid from 	wti_m_committee_activity ftm where uuid='" . $id . "'  ";
        $query = $this->db->query($sql);

        if ($query->num_rows() > 0) {
            $where_edt = "uuid = '{$id}' ";
		    $add_in['delete_status'] = 1;
		    $update_status=$this->common->updateRecord('wti_m_committee_activity',$add_in,$where_edt);
          //  $rowdata = $query->row_array();
         //   $sql = "delete from 	wti_t_newsblogs where uuid='" . $id . "'  ";
          //  $this->db->query($sql);
            //$this->common->createjson('wti_t_newsblogs', 'news');
