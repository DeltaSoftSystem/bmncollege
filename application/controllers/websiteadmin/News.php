...
class News extends CI_Controller
{
    //   public $db;
    public $title = "News";
    public $controller = "websiteadmin/news";
    ...
    public function listshow($start = 0, $otherparam = "")
    {
        ...
        $data['add_link'] = $this->controller . '/news_action';
        $data['edit_link'] = $this->controller . '/news_action';
        ...
        $this->load->view('websiteadmin/news_listall_view', $data);
        $this->session->unset_userdata('success');
        $this->session->unset_userdata('warning');
        $this->session->unset_userdata('error');
    }

    public function news_action($id = "")
    {
    ...
    public function delete_data($id = 0)
    {
        ...
        $sql = "select  uuid from 	wti_t_newsblogs ftm where uuid='" . $id . "'  ";
        $query = $this->db->query($sql);

        if ($query->num_rows() > 0) {
            $where_edt = "uuid = '{$id}' ";
            $add_in['delete_status'] = 1;
            $update_status = $this->common->updateRecord('wti_t_newsblogs', $add_in, $where_edt);
