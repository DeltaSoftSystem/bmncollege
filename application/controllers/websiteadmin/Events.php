...
class Events extends CI_Controller
{
    //   public $db;
    public $title = "Events";
    public $controller = "websiteadmin/events";
    ...
    public function listshow($start = 0, $otherparam = "")
    {
        ...
        $data['activaation_id'] = 1012;
        $data['sub_activaation_id'] = '1012_2';
        $data['title'] = 'Event';
        $data['start'] = $start;
        $data['maxm'] = $maxm = 50;
        ...
        $data['add_link'] =  $this->controller . '/events_action';
        $data['edit_link'] =  $this->controller . '/events_action';
        ...
        $this->load->view('websiteadmin/events_listall_view', $data);
    ...
    public function events_action($id = "")
    {
        ...
        $data['fun_name'] = 'events_action/' . $id;
        $data['controller'] = $this->controller;
        ...
        if (isset($_POST['mode']) && $_POST['mode'] == "edit_content") {
            ...
            if (isset($_FILES['main_image']['name']) && $_FILES['main_image']['name'] != '') {
                ...
                $filename = "events-" .time(). $this->common->tep_short_name_list($_FILES["main_image"]['name']);
            ...
            if ($error == '') {
              ...
              if ($heading_old != $heading) {
                  $add_in['slug_name'] = $this->common->getUniqueSlug('wti_m_events', 'slug_name', $this->common->tep_short_name_list($heading), 'slug_name');
              }
                ...
                if ($id != "") {
                  ...
                  $where = "uuid = '" . $id . "'";
                  $update_status = $this->common->updateRecord('wti_m_events', $add_in_m, $where);
                ...
                if($add_in_m['status_flag']==1){
                    ...
                    $add_in_search['section_name'] = "Events";
                    ...
                    $add_in_search['urls'] = site_url('upcoming-events');
                    $add_in_search['main_url'] =  site_url('upcoming-events');
                ...
                $blogs_id = $this->common->insertRecord('wti_m_events', $add_in_m);
              ...
              $this->common->createjson('wti_m_events', 'events','select *    from   wti_m_events b  	where   b.delete_status=0 and status_flag=1 order by  b.newsblogs_id desc limit 0,3','multiple','home');
        ...
        $data_info = array();
        if ($id != "") {
            $sql = "select * from  wti_m_events b where   b.delete_status=0 and b.uuid='" . $id . "'   order by  b.newsblogs_id desc";
            $query = $this->db->query($sql);
            if ($query->num_rows() > 0) {
            ...
            if (isset($add_in_m) && sizeof($add_in_m) > 0) {
                ...
                $sql = "select count('')  as numrows  from wti_m_events        where     (delete_status=0 or delete_status IS NULL)   ";
                $query = $this->db->query($sql);
                $resultdata = $query->row_array();
        ...
        $this->load->view('websiteadmin/events_adddata', $data);
        $this->session->unset_userdata('success');
        $this->session->unset_userdata('warning');
        $this->session->unset_userdata('error');
    ...
    public function delete_data($id = 0)
    {
        ...
        $sql = "select  uuid from 	wti_m_events ftm where uuid='" . $id . "'  ";
        $query = $this->db->query($sql);

        if ($query->num_rows() > 0) {
            $where_edt = "uuid = '{$id}' ";
		    $add_in['delete_status'] = 1;
		    $update_status=$this->common->updateRecord('wti_m_events',$add_in,$where_edt);
