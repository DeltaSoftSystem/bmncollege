...
class Admissionenquiry extends CI_Controller
{
    //   public $db;
    public $title = "Admission enquiry";
    public $controller = "websiteadmin/admissionenquiry";
    public $m_act = 0;
    ...
    public function listall($start = 0, $otherparam = "")
    {
        ...
        $data['sub_activaation_id'] = '1105_2';
        ...
        $resultdata = array();
        $sql = "select count('')  as numrows  from wti_t_admission_enquiry c      where     delete_status=0 order by id desc ";
        $query = $this->db->query($sql);
        $resultdata = $query->row_array();
        $data['num_row'] = $resultdata['numrows'] ;//= $this->common->numRow($this->tablename,$where_cond);
         
        
        $this->load->view('websiteadmin/admissionenquiry', $data);
    ...
    public function download(){
        ...
        $csv_name = 'admission_enquiry-'.time();
        ...
        // $date_range = (isset($_GET['date_range'])) ? $this->input->get('date_range') : '';
            ...
            // $date_range_exp[1] = $this->common->getDateFormat(trim($date_range_exp[1]), "Y-m-d");
                ...
                fputcsv($output, array('Date', 'Full_Name', 'Contact_Number','Email','City','Program','Message'));

             $sql_sub = "";   
              //  $sql_sub = " and  (DATE_FORMAT(m.datum,'%Y-%m-%d')   BETWEEN '" . $date_range_exp[0] . "' AND '" . $date_range_exp[1] . "') ";
                $sql = "select  add_date,full_name,contact_number,email_addrress,city,program ,message from  wti_t_admission_enquiry m    where     delete_status=0 order by id desc";
                    $query = $this->db->query($sql);
    ...
    public function editdata($id = 0)
    {
        ...
        if (isset($_POST['mode']) && $_POST['mode'] == "edit_content") {
            ...
            if ($error == '') {
                ...
                $update_status = $this->common->updateRecord('wti_t_admission_enquiry', $add_in, $where_edt);
        ...
        $data_info = array();
        if ($id != "") {
            $sql = "select  * from wti_t_admission_enquiry ftm where id='" . $id . "'  ";
            $query = $this->db->query($sql);
            if ($query->num_rows() > 0) {
        ...
        $this->load->view('websiteadmin/admissionenquiry_action', $data);
    ...
    public function delete_data($id = 0)
    {
        ...
        $sql = "select  id from  wti_t_admission_enquiry where id='" . $id . "'  ";

        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {

            $sql = "update wti_t_admission_enquiry  set  delete_status=1  where id='" . $id . "'";
            $this->db->query($sql);
