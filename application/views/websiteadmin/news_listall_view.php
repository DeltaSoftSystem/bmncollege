                        ...
                        <table class="table table-hover datatable-responsive">
                            ...
                            <tbody>
                                <?php
									  ...
									  ?>
                                <tr
                                    class="  border-left-3  <?php echo ($status == "1") ? 'border-left-success' : 'border-left-danger' ?>  tr<?php echo $this->common->getDbValue($value['status_flag']); ?>">
                                    <td><?php echo $i; ?></td>
                                    <td><?php echo substr($this->common->getDbValue($value['heading']),0,100); ?></td>

                                    <!-- <td><?php echo $this->common->getDbValue($value['cate_name']); ?></td> -->
                                    <td><?php echo isset($value['event_dates']) ? $this->common->getDateFormat($value['event_dates'], DATE_FORMAT_PHP) : ''; ?>
                                    </td>
                                    <td>
                                        <?php
...
if ($status != "1") {echo '<span class="badge badge-danger  w-1001">Inactive</span>';}
      ?></td>
                                    <td><a href="<?php echo site_url($edit_link.'/'.$value['uuid'])?>"
                                            class="list-icons-item text-primary-600" data-popup="tooltip" title="Edit"
                                            data-container="body"><i class="icon-pencil7"></i></a>
                                        <a href="#" data-uuid="<?php echo $value['uuid'] ?>"
                                            class="list-icons-item text-danger-600 bootbox_custom" data-popup="tooltip"
                                            title="Delete" data-container="body"><i class="icon-trash"></i></a>
